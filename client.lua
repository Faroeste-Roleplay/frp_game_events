...
local eventHashes = {
	...
	EVENT_CRIME_CONFIRMED = `EVENT_CRIME_CONFIRMED`,
	EVENT_CRIME_REPORTED = `EVENT_CRIME_REPORTED`,
	...
	EVENT_CRIME_CRY_FOR_HELP = `EVENT_CRIME_CRY_FOR_HELP`,
	...
	EVENT_SHOCKING_CRIME_SCENE = `EVENT_SHOCKING_CRIME_SCENE`,
...
local eventHashes2 = {
	...
	-- 'EVENT_SHOCKING_ANIMAL_CHARGING',
...
local type = {
    ...
    "MpAmbient",
    "MpVehicleConvoy",
    "MpDefaultNoShotFired",
    "MpStealthHardNoShotFired",
    "MpMissionGiver",
    "MpHearingRangeReaction"
}
gShockingEvent = nil
...
AddEventHandler("onResourceStop", function( resName )
	if resName == GetCurrentResourceName() then
		if gShockingEvent then
			RemoveShockingEvent(gShockingEvent)
		end
...
CreateThread(function()
	...
	local struct = DataView.ArrayBuffer(8 )
	
	struct:SetInt32(8 * 0, GetHashKey("EVENT_SHOCKING_WITNESS_CALL_FOR_LAW"))
	struct:SetInt32(8 * 1, GetHashKey("EVENT_SHOCKING_WITNESS_REPORTING"))
	struct:SetInt32(8 * 2, GetHashKey("EVENT_SHOCKING_CRIME_SCENE"))
	struct:SetInt32(8 * 3, GetHashKey("EVENT_SHOCKING_PURSUIT"))
	-- struct:SetInt32(8 * 4, -1)
	-- struct:SetInt32(8 * 5, -1)
	-- struct:SetInt32(8 * 6, -1082130432)
	-- struct:SetInt32(8 * 7, -1082130432)
	-- struct:SetInt32(8 * 8, -1082130432)
	-- struct:SetInt32(8 * 9, -1082130432)
	-- struct:SetInt32(8 * 10, -1082130432)
	-- struct:SetInt32(8 * 11, -1082130432)
	-- struct:SetInt32(8 * 12, -1)
	-- struct:SetInt32(8 * 13, -1)
	-- struct:SetInt32(8 * 14, -1)
	-- struct:SetInt32(8 * 15, -1)

	-- local buff = struct:Buffer()
	-- print(" BUFFER :: ", buff)

	-- local ret, atg = Citizen.InvokeNative(0xCA1315C33B9A2847, struct:Buffer(), Citizen.ReturnResultAnyway())
	-- print(" RET :: ", ret, atg)
    ...
    while true do
		...
		-- 	for _, key in pairs( eventHashes2 ) do 
		-- 		local isPedResponding = IsPedRespondingToEvent(ped, GetHashKey(key))

		-- 		local isEventInQueue = IsEventInQueue(ped, GetHashKey(key))

		-- 		if isEventInQueue ~= 0 then
		-- 			print(" isEventInQueue :: ", isEventInQueue, key)
					
		-- 		end

		-- 		if isPedResponding then 
					
		-- 			print(" OT :: ", isPedResponding, key)
		-- 		end
		-- 	end
		-- end
		...
		if size > 0 then
			for i = 0, size - 1 do
				...
				for key, event in pairs( eventHashes ) do 

					if event == eventAtIndex or GetHashKey( key ) == eventAtIndex then

						if string.find(key, "CRIME") then
							local eventData = GetEventData(0, i, 3)
							print(key, eventAtIndex, json.encode(eventData) )
						end
					end

                    -- if event == eventAtIndex then
                    --     print(" eventAtIndex : ", eventAtIndex)
                    -- end 
					...
					-- if isPedResponding then 
                    --     -- if 
                    --     --     key == "EVENT_SHOCKING_WITNESS_CALL_FOR_LAW" or
                    --     --     key == "EVENT_SHOCKING_WITNESS_REPORTING" or
                    --     --     key == "EVENT_SHOCKING_PURSUIT" or
                    --     --     key == "EVENT_SHOCKING_CRIME_SCENE" or
                    --     --     -- key == "EVENT_SHOCKING_PLAYER_RUNNING" or
                    --     --     key == "EVENT_CHALLENGE_GOAL_UPDATE"
                    --     -- then
                    --     --     break 
                    --     -- end
                    --     print(" OT :: ", isPedResponding, key)
					...
					-- -- local isPedRespo = IsPedRespondingToThreat(ped )
					-- local isEventInQueue = IsEventInQueue(ped, GetHashKey(key))

					-- if isEventInQueue ~= 0 then
					-- 	print(" IsEventInQueue :: ",isEventInQueue)
					-- end

                    -- local id = IsShockingEventInSphere( GetHashKey(key), playerPedCoords, 100.0 )

                    -- -- local other = IsEventTrackerActive("MpAmbient", GetHashKey(key))

					-- -- if other ~= 0 and other then
					-- 	-- print(" other :: ", other, key)
					-- -- end
					...
					-- -- if eventAtIndex == GetHashKey(key) then
						
					-- 	if 
					-- 		eventAtIndex == `EVENT_PED_DESTROYED` or
					-- 		eventAtIndex == `EVENT_PED_CREATED` or
					-- 		eventAtIndex == `EVENT_ENTITY_BROKEN` or
					-- 		eventAtIndex == `EVENT_VEHICLE_DESTROYED` or
					-- 		eventAtIndex == `EVENT_VEHICLE_CREATED` or
					-- 		eventAtIndex == `EVENT_CHALLENGE_GOAL_UPDATE`
					-- 	then
					-- 		break 
					-- 	end
					-- 	-- print(" Event :: ", key)
