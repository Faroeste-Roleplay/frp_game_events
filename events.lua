...
local getEventData = function(eventName)
    ...
    for i = 0, size - 1 do
        ...
        local dataExists = Citizen.InvokeNative(0x57EC5FA4D4D6AFCA, eventGroup, i, eventDataStruct:Buffer(),
            eventDataSize)
...
Citizen.CreateThread(function()
    while true do
        local currentEventName = events.listenForAnyEvents({})
        if currentEventName then 
            local eventData = events.getEventData(currentEventName)
            TriggerEvent(("gameEvent:%s"):format(currentEventName), eventData)
            TriggerEvent('gameEvent', currentEventName, eventData)
        end
        Wait(0)
    end
end)